...
export default function DashboardLayout({
  ...
  return (
    <section className="min-h-screen max-h-screen flex">
      {/* Sidebar */}
      <aside className="w-64 3xl:w-72 bg-white p-5 3xl:p-7 shrink-0">
        {/* Logo */}
        <figure
          onClick={() => router.push("/")}
            ...
            return (
              <div key={idx}>
                <Link
                  key={link.id}
                  href={link.path}
                  className={`flex px-4 3xl:px-5 py-3 rounded-[50px] gap-2.5 3xl:gap-4 items-center font-medium
                     ${
                       isActive
      ...
      <main className="flex-grow overflow-x-hidden">
        ...
        {/* outlet */}
        <section className="p-6 3xl:p-7 bg-gray-100 h-[calc(100vh-84px)] overflow-y-auto">
          {children}
        </section>
      </main>
    </section>
